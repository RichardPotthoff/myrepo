...
Eprom=[simulateEprom(address,actions) for address in range(1<<9)]
...
CounterEprom=[simulateEprom(address,actions) for address in range(1<<9)]
...
#      newstate=actions[state&0x7F][phase%4]
      newstate=Eprom[state&0x7F|(intToGray((-phase)%4)<<7)]
