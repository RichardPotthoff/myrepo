...
#					if key!=invalidkey and key!=releasekey:
						data=0b110
...
class Game(Scene):
	def setup(self):
		self.keypad=Keypad()
		self.digiLock=DigiLock()
		self.Eprom=[0xFF]*(1<<12)
		for i in range(1<<12):
			self.Eprom[i]=self.digiLock.nextstate(i)
		self.location=None
		self.locations=[]
		self.score_label = LabelNode('0', font=('Avenir Next', 40), position=(self.size.w/2, self.size.h-50),alpha=1.0, parent=self)
		self.state=0b00000
		w=self.size.w
		h=self.size.h
		nh=4
		...
		for i in range(nh):
			for j in range(nv):
				n=i*nv+j+1
				LabelNode('%X'%(n), font=('Avenir Next', 60), position=(w/2.0+dv*(j-nv/2+0.5), h/2.0-dh*(i-nh/2+0.5)),alpha=0.7, parent=self)
		hLasers=[(Point(w,h/2.0-dh*(i-nh/2+0.5)),w,5,180,'red') for i in range(nh)]
		vLasers=[(Point(w/2.0+dv*(i-nv/2+0.5),0),h,5,90.0,'lightgreen') for i in range(nv)]
	...
	def closestTapDistance(self,P):
		result=max(self.size)*2
		for l in self.locations:
			result=min(result,max(abs(l.location.x-P.x),abs(l.location.y-P.y)))
		return result
