...
class Game(Scene):
  ...
  def setup(self):
    ...
    hLasers=[(Point(w,h/2.0-dh*(i-nh/2+0.5)),w,0.0065*min(self.size),120,'red') for i in range(nh)]
    vLasers=[(Point(w/2.0+dv*(i-nv/2+0.5),0),h,0.0065*min(self.size),60.0,'lightgreen') for i in range(nv)]
