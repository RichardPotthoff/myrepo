...
class Piano(Scene):
  ...
  def setup(self):
    ...
    key_textures = [
      Texture(img) for img in make_key_images(0.5 * r, r, scale=1.0)
    ]
    ...
    for i in range(12):
      ...
      key = Key(
        SpriteNode(key_textures[texture_index],
                   scale=1.0,
                   position=(self.size.w / 2 + cos(ang) * 0.74 * r,
                             self.size.h / 2 + sin(ang) * 0.74 * r)))
      key.node.rotation = ang
      [key.texture,
       key.highlight_texture] = key_textures[texture_index:texture_index + 2]
