...
import re
import collections
#print('[\n"'+'",\n"'.join(' '.join(reminder.title.strip().split(' ')[:-1])for reminder in reminders.get_reminders({calendar.title: calendar for calendar in reminders.get_all_calendars()}['Groceries']))+'",\n]') 
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            pcl_header = (
                ...
                #b"\x1b(s10V"
                +f'\x1b(s{int(proportional)}p{pointsize}v{style}s{weight}b{font_lookup[font]}T'.encode()+
...
def main(args):
    ListName = args[1] if len(args) > 1 and not args[1].startswith('-') else 'Groceries'
    ...
    success = send_to_printer(data_roman8,testing='--testing' in args)
...
def parse_grocery_item_title(item):
  pattern = re.compile(r'''
    ^\s*                          # Optional leading spaces
    (?P<title>[^0-9$]*?)          # Title: anything except digits or $, non-greedy
    \s*                           # Optional spaces
    (?P<count>\d*)                 # Optional count (e.g., 2 or empty)
    \s*                           # Optional spaces
    (?:\$(?P<price>[\d.,]+(?:/\w+)?))?  # Optional price: $ followed by number, optional /unit
    \s*$                          # Optional trailing spaces
''', re.VERBOSE)
  #match=re.match(r'(?P<title>[^0-9$]*)(?P<count>\d*)(?:\$(?P<price>.*)$)',item)
...
class GroceryItem():
  def __init__(self,reminder_title):
    ...
    try:
      ...
      self.price=0.0
  def __repr__(self):
    return f'{self.__class__.__name__}("{self.title} {self.count}${self.price}")'
...
if __name__ == '__main__':
    ...
    print(f'argv: {args}')
    ListName = args[1] if len(args) > 1 and not args[1].startswith('-') else 'Groceries'
    #manually sorted by section in store
    item_list_sorted=[
    "Apples",
    "Sugar",
    "Kakao",
    ...
    item_dict=collections.defaultdict(None,((i,None) for i in item_list_sorted))#pre-sorted dict
    
    groceries_calendar={calendar.title: calendar for calendar in reminders.get_all_calendars()}[ListName]
    
    todo=reminders.get_reminders(groceries_calendar,completed=False)
    
    for reminder in todo:
      item=GroceryItem(reminder.title)# reminder title has the form 'title count$price' e.g. 'Jogurt 6$0.52' 
      item_dict[item.title]=item
      
    items=[item for item in item_dict.values() if item !=None ]#remove items not on the list, but keep sorting order
    
    total=sum(item.count*item.price for item in items)
    # Format shopping list as plain text, only items, with carriage returns
    data='\r\n'
    dots_per_mm = 300/25.4  # 1mm ≈ 11.81 dots (300 DPI)
    dots_per_point = 300/72 # 300 dots = 72
    pointsize=12
    linespacing=1.25
    style=4
    dots_per_digit = {0:0.56,4:0.46}[style]*pointsize*dots_per_point #
    top_margin=3.0
    x_left_margin=0.0
    x_count_tab=35.0
    x_price_tab=48.0
